...
const IncidentStatusCard: React.FC<IncidentStatusCardProps> = ({
  ...
  const statusCounts = statusLevels.map((level) => ({
    ...level,
    count: incidents.filter((i) => i.status === level.key && !i.archived)
      .length,
  }));
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Stack
        className="vw-incident-status"
        sx={incidentManagementCard}
        direction="row"
      >
        {statusCounts.map((level) => (
          <Tooltip
            key={level.key}
            title={`${level.count} ${level.label} Incident${
              level.count !== 1 ? "s" : ""
            }`}
            arrow
            placement="top"
          >
            ...
            >
              <Typography sx={incidentManagementCardKey}>
                {level.label}
              </Typography>
              <Typography sx={incidentManagementCardValue}>
                {level.count}
              </Typography>
            </Stack>
          </Tooltip>
