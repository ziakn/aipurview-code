...
function calculateConnectionCounts(data: EntityGraphData): Map<string, number> {
  ...
  const regularUseCases = useCases.filter(
    (p) => !(p as { is_organizational?: boolean }).is_organizational,
  );

  regularUseCases.forEach((uc) => {
    const ucId = `useCase-${uc.id}`;
    models.forEach((m) => {
      if (m.projects?.includes(uc.id)) add(ucId);
    });
    risks.forEach((r) => {
      if (r.project_id === uc.id) add(ucId);
    });
  });
...
export function generateNodesAndEdges(
  ...
  const { centerX, centerY, useCaseRadius, modelRadius, vendorRadius, riskRadius, evidenceRadius } =
    layoutConfig;
  ...
  const shouldShowEdge = (label: string) =>
    visibleRelationships.length === 0 || visibleRelationships.includes(label);
  ...
  if (visibleEntities.includes("useCases")) {
    regularUseCases.forEach((uc, i) => {
      ...
      addNode(
        ...
        {
          sublabel: uc.project_title,
          hasHighRisk: hasRisk,
          rawData: { ...uc, owner_name: getUserName(uc.owner) },
        },
      );
    });
  }
    ...
    models.forEach((m, i) => {
      ...
      addNode(
        ...
        hasRisk ? entityColors.risk : entityColors.model,
        {
          ...
          status: m.status,
          hasHighRisk: hasRisk,
          rawData: { ...m, owner_name: getUserName(m.owner) },
        },
      );
    ...
    vendors.forEach((v, i) => {
      ...
      addNode(
        ...
        hasRisk ? entityColors.risk : entityColors.vendor,
        {
          ...
          rawData: {
            ...v,
            owner_name: getUserName(v.owner),
            assignee_name: getUserName(v.assignee),
            reviewer_name: getUserName(v.reviewer),
          },
        },
      );
      ...
      if (
        visibleEntities.includes("useCases") &&
        v.projects?.length &&
        shouldShowEdge("supplies")
      ) {
    ...
    risks.forEach((r, i) => {
      ...
      addNode(
        ...
        entityColors.risk,
        {
          ...
          riskSource: r.source,
          hasHighRisk: isHigh,
          rawData: { ...r, action_owner_name: getUserName(r.action_owner) },
        },
      );
      ...
      if (shouldShowEdge("affects")) {
        if (
          r.model_id &&
          visibleEntities.includes("models") &&
          createdNodeIds.has(`model-${r.model_id}`)
        ) {
          edges.push(
            createEdge(
              `risk-${r.id}-model-${r.model_id}`,
              id,
              `model-${r.model_id}`,
              "affects",
              entityColors.risk,
        ...
        if (
          r.project_id &&
          visibleEntities.includes("useCases") &&
          createdNodeIds.has(`useCase-${r.project_id}`)
        ) {
          edges.push(
            createEdge(
              `risk-${r.id}-useCase-${r.project_id}`,
              id,
              `useCase-${r.project_id}`,
              "affects",
              entityColors.risk,
            ),
          );
        ...
        if (
          r.vendor_id &&
          visibleEntities.includes("vendors") &&
          createdNodeIds.has(`vendor-${r.vendor_id}`)
        ) {
          edges.push(
            createEdge(
              `risk-${r.id}-vendor-${r.vendor_id}`,
              id,
              `vendor-${r.vendor_id}`,
              "affects",
              entityColors.risk,
...
export function getConnectedEntities(
  ...
  edges.forEach((edge) => {
    ...
    const label =
      (entity as { project_title?: string }).project_title ||
