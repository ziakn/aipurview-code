...
describe("MicrosoftSignIn", () => {
  ...
  it("renders with custom text", () => {
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} text="Sign in with SSO" />,
    );
  ...
  it("shows not-configured message when tenantId is missing", () => {
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} tenantId={undefined} />,
    );
  ...
  it("shows not-configured message when clientId is missing", () => {
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} clientId={undefined} />,
    );
    expect(screen.getByText("Microsoft Sign-In Not Configured")).toBeInTheDocument();
  });
  ...
  it("shows not-configured message when organizationId is missing", () => {
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} organizationId={undefined} />,
    );
    expect(screen.getByText("Microsoft Sign-In Not Configured")).toBeInTheDocument();
  });
  ...
  it("disables button when isSubmitting is true", () => {
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} isSubmitting={true} />,
    );
  ...
  it("opens popup window on click with correct URL", () => {
    ...
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} setIsSubmitting={setIsSubmitting} />,
    );
    ...
    expect(windowOpen).toHaveBeenCalledWith(expectedUrl, "vw_microsoft_sso", "width=500,height=650");
    ...
    act(() => {
      window.dispatchEvent(
        new MessageEvent("message", {
          data: { type: "MICROSOFT_AUTH_SUCCESS", token: "test-token", expirationDate: "2026-12-31" },
  ...
  it("ignores messages from different origin", () => {
    const onError = vi.fn();
    renderWithProviders(
      <MicrosoftSignIn {...defaultProps} onError={onError} />,
    );
    act(() => {
      window.dispatchEvent(
        new MessageEvent("message", {
