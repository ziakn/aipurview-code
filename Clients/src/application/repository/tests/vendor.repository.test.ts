...
describe("vendor.repository", () => {
  ...
  describe("getAllVendors", () => {
    it("should make GET request to /vendors", async () => {
      const mockResponse = {
        data: [
          { id: 1, name: "Vendor 1" },
          { id: 2, name: "Vendor 2" },
    ...
    it("should pass signal when provided", async () => {
      ...
      expect(apiServices.get).toHaveBeenCalledWith("/vendors", { signal });
    });
  ...
  describe("getVendorById", () => {
    it("should make GET request to /vendors/:id", async () => {
      const mockResponse = {
        data: { id: 1, name: "Vendor 1" },
        status: 200,
        statusText: "OK",
    ...
    it("should throw error without response property for network errors", async () => {
      ...
      await expect(getVendorById({ id: 1 })).rejects.toThrow("Connection refused");
    });
  });

  describe("getVendorsByProjectId", () => {
    it("should make GET request to /vendors/project-id/:projectId", async () => {
      const mockResponse = {
        data: [{ id: 1, name: "Vendor 1" }],
        status: 200,
        statusText: "OK",
    ...
    it("should throw error without response property for network errors", async () => {
      ...
      await expect(getVendorsByProjectId({ projectId: 1 })).rejects.toThrow("Network timeout");
    });
  });
  ...
  describe("createNewVendor", () => {
    it("should make POST request to /vendors with body", async () => {
      ...
      const result = await createNewVendor({ body });
  ...
  describe("update", () => {
    it("should make PATCH request to /vendors/:id with body", async () => {
      ...
      const result = await update({ id: 1, body });
