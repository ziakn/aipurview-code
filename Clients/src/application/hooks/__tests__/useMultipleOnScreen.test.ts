...
describe("useMultipleOnScreen", () => {
  it("returns refs array with the correct length", () => {
    const { result } = renderHook(() => useMultipleOnScreen<HTMLDivElement>({ countToTrigger: 3 }));
  ...
  it("allVisible is false initially", () => {
    const { result } = renderHook(() => useMultipleOnScreen<HTMLDivElement>({ countToTrigger: 2 }));
