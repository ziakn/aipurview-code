...
- No AIPurview React components, MUI theme tokens, layouts, or Style Guide patterns need to be created or modified.
- Swagger UI is a third-party rendering layer and is out of scope for the AIPurview design system unless a future decision is made to embed API docs inside the React client.
...
- **Discovery:** There is no in-app link to `/api/docs` in the AIPurview React client. Engineers, QA, and security reviewers must know the URL or bookmark it.
...
- **Layout / branding:** Swagger UI does not inherit AIPurview typography, color palette, or navigation. This is acceptable for an internal developer tool but should be acknowledged if external stakeholders or auditors are expected to use it.
...
No new AIPurview UI means no new accessibility work is required for this initiative.
