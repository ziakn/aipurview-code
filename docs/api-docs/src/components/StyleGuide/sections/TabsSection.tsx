...
// Mock TabItem interface to match the original TabBar component
interface TabItem {
  label: string;
  value: string;
  icon?: string;
...
// MockTabBar component - visually mimics the real TabBar using MUI Box components
...
const MockTabBar: React.FC<MockTabBarProps> = ({
  tabs,
  activeTab,
  onChange,
  ...
  const handleTabClick = (value: string, disabled?: boolean) => (event: React.MouseEvent) => {
    ...
    >
      {tabs.map((tab) => {
        ...
        return (
          <Box
            key={tab.value}
            onClick={handleTabClick(tab.value, isDisabled)}
            sx={{
              position: "relative",
              display: "flex",
              alignItems: "center",
              ...
              "&:hover": {
                color: !isDisabled ? indicatorColor : theme.palette.text.disabled,
              },
              "&::after": {
                content: '""',
                position: "absolute",
          ...
          >
            {/* Icon placeholder - just show text since we don't have icon mapping */}
            {tab.icon && (
              <Box
                sx={{
                  fontSize: "14px",
            ...
            {/* Count badge */}
            {tab.count !== undefined && !tab.isLoading && (
              <Box
                sx={{
                  display: "inline-flex",
                  alignItems: "center",
...
// MockTabContext component - simple wrapper for state management
const MockTabContext: React.FC<{ value: string; children: React.ReactNode }> = ({
  children,
}) => {
    ...
    <Box sx={{ p: "32px 40px" }}>
      ...
      <SpecSection title="Basic tabs">
        <Typography sx={{ fontSize: 13, color: theme.palette.text.tertiary, mb: "24px" }}>
          The TabBar component wraps MUI TabList with AIPurview styling.
