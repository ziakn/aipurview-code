...
export async function checkOrganizationalProjectExists(
...
export async function insertData(insertQuery: string, transaction: Transaction): Promise<any> {
  const result = await sequelize.query(insertQuery, { transaction });
  return result;
}

export async function deleteDemoVendorsData(
  organizationId: number,
  transaction: Transaction,
): Promise<void> {
  const result = await sequelize.query(
  ...
  // Delete demo vendors
  await sequelize.query(
    `DELETE FROM vendors WHERE organization_id = :organizationId AND is_demo;`,
    { replacements: { organizationId }, transaction },
  );
}
