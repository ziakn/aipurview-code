...
export const getAllAutomationTriggersQuery = async () => {
  const result = await sequelize.query(`SELECT * FROM automation_triggers`, {
    mapToModel: true,
...
export const getAutomationByIdQuery = async (id: number, organizationId: number) => {
  ...
  const automation = automations[0][0] as unknown as ITenantAutomationAction & {
    actions: ITenantAutomationAction[];
  };
  automation.actions = [];
...
export const createAutomationQuery = async (
  ...
  const _automation = (await sequelize.query(
      ...
      :organizationId, :name, :trigger_id, :params, :userId) RETURNING *;`,
    {
      replacements: {
        organizationId,
        ...automationMeta,
        params: JSON.stringify(automationMeta.params),
        userId,
      },
      transaction,
    },
  )) as [ITenantAutomationAction[], number];
  const automation = _automation[0][0] as unknown as ITenantAutomationAction & {
    actions: ITenantAutomationAction[];
  };
  ...
  const _actions = (await sequelize.query(
    `SELECT * FROM automation_actions_data WHERE organization_id = :organizationId AND automation_id = :id ORDER BY "order";`,
    {
      replacements: { organizationId, id: automation.id },
      transaction,
    },
  )) as [TenantAutomationActionModel[], number];
...
export const updateAutomationByIdQuery = async (
  ...
  const setClause = ["name", "trigger_id", "params", "is_active"]
    .filter((field) => {
      ...
      if (field === "params" && currentAutomation[field as keyof IAutomation] !== undefined) {
        (updatedAutomation as any)[field] = JSON.stringify(
          currentAutomation[field as keyof IAutomation],
        );
        return true;
      }
      if (
        currentAutomation[field as keyof IAutomation] !== undefined &&
        currentAutomation[field as keyof IAutomation]
      ) {
        (updatedAutomation as any)[field] = currentAutomation[field as keyof IAutomation];
        return true;
  ...
  if (!setClause) {
    ...
    automation = result[0][0] as unknown as ITenantAutomationAction & {
      actions: ITenantAutomationAction[];
    };
  } else {
    ...
    const result = (await sequelize.query(query, {
      replacements: updatedAutomation,
      transaction,
    })) as [ITenantAutomationAction[], number];
    automation = result[0][0] as unknown as ITenantAutomationAction & {
      actions: ITenantAutomationAction[];
...
export const deleteAutomationByIdQuery = async (
  id: number,
  organizationId: number,
  transaction: Transaction,
) => {
