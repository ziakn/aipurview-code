 ...
 */
export async function getModelInventoryChangeHistoryById(req: Request, res: Response) {
  ...
  const offset = req.query.offset
    ? parseInt(
        Array.isArray(req.query.offset) ? String(req.query.offset[0]) : String(req.query.offset),
        10,
      )
    : 0;
