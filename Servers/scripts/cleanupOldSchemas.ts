...
async function findPublicAppTables(): Promise<string[]> {
  ...
  return results.map((r) => r.tablename).filter((t) => !PROTECTED_PUBLIC_TABLES.has(t));
}
...
async function migrateSequelizeMeta(dryRun: boolean): Promise<void> {
  ...
  const verywiseHasMeta = await tableExistsInSchema("verifywise", "SequelizeMeta");
  ...
  if (!verywiseHasMeta) {
    console.log("  verifywise.SequelizeMeta does not exist — skipping row migration.");
    console.log("  (public.SequelizeMeta will be dropped with other public tables.)");
    return;
  }
  ...
  if (dryRun) {
    ...
    console.log(`  Merged SequelizeMeta rows from public into verifywise schema.`);
  }
}
...
async function run(): Promise<void> {
  ...
  console.log(
    `  Mode: ${dryRun ? "DRY RUN (pass --confirm to execute)" : "LIVE — CHANGES WILL BE MADE"}`,
  );
  console.log("=".repeat(60));
  console.log();
  ...
  try {
    ...
    if (!vwExists) {
      console.error("ERROR: 'verifywise' schema does not exist. Run migrations first.");
      process.exit(1);
    }
    ...
    if (tenantSchemas.length === 0) {
      ...
      if (dryRun) {
        console.log(`  [DRY RUN] Would drop ${tenantSchemas.length} tenant schema(s).\n`);
      } else {
        for (const schema of tenantSchemas) {
    ...
    if (publicTables.length === 0) {
      ...
      console.log(`  Found ${publicTables.length} app table(s) in public schema:`);
      ...
      if (dryRun) {
        console.log(`  [DRY RUN] Would drop ${publicTables.length} table(s) from public schema.\n`);
      } else {
        // Drop all tables in one CASCADE statement to handle FK ordering
        const tableList = publicTables.map((t) => `public."${t}"`).join(", ");
