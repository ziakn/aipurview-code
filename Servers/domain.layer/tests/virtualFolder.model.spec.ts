...
class TestVirtualFolderModel {
  ...
  parent_id?: number | null; color?: string | null; icon?: string | null;
  is_system?: boolean; created_by!: number;
  ...
  toJSON() {
    return {
      ...
      parent_id: this.parent_id, color: this.color, icon: this.icon,
      is_system: this.is_system, created_by: this.created_by,
    };
  }
}

class TestFileFolderMappingModel {
  id!: number; file_id!: number; folder_id!: number;
  assigned_by!: number; assigned_at?: Date;
  constructor(data?: any) { if (data) Object.assign(this, data); }

  toJSON() {
    return { id: this.id, file_id: this.file_id, folder_id: this.folder_id, assigned_by: this.assigned_by };
  }
}
...
describe("VirtualFolderModel", () => {
  it("should instantiate with required fields", () => {
    const f = new TestVirtualFolderModel({
      id: 1, name: "Compliance Docs", created_by: 42,
      parent_id: null, is_system: false,
    });
  ...
  it("should support nested folders", () => {
    const child = new TestVirtualFolderModel({
      id: 2, name: "EU AI Act", created_by: 42, parent_id: 1,
    });
  ...
  it("should support styling props", () => {
    const f = new TestVirtualFolderModel({
      id: 1, name: "Test", created_by: 1, color: "#FF0000", icon: "folder-open",
    });
...
describe("FileFolderMappingModel", () => {
  it("should link file to folder", () => {
    const m = new TestFileFolderMappingModel({
      id: 1, file_id: 10, folder_id: 20, assigned_by: 42,
    });
