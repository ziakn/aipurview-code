...
class TestScanModel {
  id?: number; repository_url!: string; branch?: string; status!: string;
  started_at?: Date; completed_at?: Date; total_findings?: number;
  organization_id!: number; created_by!: number;
  constructor(data?: any) { if (data) Object.assign(this, data); }
}

class TestFindingModel {
  id?: number; scan_id!: number; file_path!: string; line_number?: number;
...
describe("ScanModel", () => {
  it("should instantiate with required fields", () => {
    const scan = new TestScanModel({
      id: 1, repository_url: "https://github.com/org/repo",
      branch: "main", status: "completed", total_findings: 5,
  ...
  it("should handle optional fields", () => {
    const scan = new TestScanModel({
      ...
      status: "pending", organization_id: 1, created_by: 1,
    });
...
describe("FindingModel", () => {
  it("should instantiate with required fields", () => {
    const finding = new TestFindingModel({
      id: 1, scan_id: 10, file_path: "src/model.py",
      line_number: 42, finding_type: "ai_usage", severity: "high",
      description: "Unvetted AI model usage", confidence_score: 0.95,
    });
  ...
  it("should link to scan via scan_id", () => {
    const finding = new TestFindingModel({ scan_id: 10, file_path: "a.py", finding_type: "t", severity: "low" });
