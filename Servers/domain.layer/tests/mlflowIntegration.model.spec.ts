...
class TestMlflowIntegrationModel {
  id?: number; organization_id!: number; tracking_uri!: string;
...
describe("MlflowIntegrationModel", () => {
  it("should instantiate with connection config", () => {
    const m = new TestMlflowIntegrationModel({
      id: 1, organization_id: 1, tracking_uri: "http://mlflow.example.com:5000",
      is_active: true,
    });
  ...
  it("should handle inactive state", () => {
    const m = new TestMlflowIntegrationModel({
      organization_id: 1, tracking_uri: "http://localhost:5000", is_active: false,
    });
