...
jest.mock("sequelize-typescript", () => ({
  Column: jest.fn(), DataType: {
    ...
    DATE: "DATE", BOOLEAN: "BOOLEAN", ARRAY: jest.fn(), ENUM: jest.fn(), NOW: "NOW",
  },
...
class TestSlackWebhookModel {
  id?: number; access_token!: string; access_token_iv?: string;
  scope!: string; user_id?: number; team_name!: string;
  team_id!: string; channel!: string; channel_id!: string;
  configuration_url!: string; url!: string; url_iv?: string;
  is_active?: boolean; routing_type?: SlackNotificationRoutingType[];
  created_at?: Date;
  ...
  toJSON() {
    return {
      id: this.id, scope: this.scope, team_name: this.team_name,
      channel: this.channel, is_active: this.is_active, routing_type: this.routing_type,
    };
  }
}
...
describe("SlackWebhookModel", () => {
  it("should instantiate with required fields", () => {
    const s = new TestSlackWebhookModel({
      id: 1, access_token: "enc_token", scope: "chat:write",
      team_name: "Acme", team_id: "T123", channel: "#alerts",
      channel_id: "C123", configuration_url: "https://slack.com/config",
      url: "https://hooks.slack.com/xxx", is_active: true,
    });
  ...
  it("should store routing types", () => {
    const s = new TestSlackWebhookModel({
      access_token: "t", scope: "s", team_name: "T", team_id: "T1",
      channel: "#c", channel_id: "C1", configuration_url: "u", url: "u",
      routing_type: [
  ...
  it("should serialize to JSON without sensitive fields", () => {
    const s = new TestSlackWebhookModel({
      id: 1, access_token: "secret", scope: "s", team_name: "T",
      channel: "#c", is_active: true, team_id: "T1", channel_id: "C1",
      configuration_url: "u", url: "u",
    });
    const json = s.toJSON();
