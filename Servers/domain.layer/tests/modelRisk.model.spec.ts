...
class TestModelRiskModel {
  id?: number; model_inventory_id!: number; risk_name!: string;
  description?: string; risk_level!: string; status!: string;
  category?: string; mitigation_plan?: string;
...
describe("ModelRiskModel", () => {
  it("should instantiate with required fields", () => {
    const r = new TestModelRiskModel({
      id: 1, model_inventory_id: 10, risk_name: "Data leakage",
      risk_level: ModelRiskLevel.HIGH, status: "Open",
    });
  ...
  it("should support all risk levels", () => {
    Object.values(ModelRiskLevel).forEach((level) => {
      const r = new TestModelRiskModel({
        model_inventory_id: 1, risk_name: "Test", risk_level: level, status: "Open",
      });
  ...
  it("should handle optional fields", () => {
    const r = new TestModelRiskModel({
      model_inventory_id: 1, risk_name: "Test", risk_level: "Low", status: "Open",
    });
