jest.mock("sequelize-typescript", () => ({
  Column: jest.fn(), DataType: {
    ...
    JSONB: "JSONB", NOW: "NOW", STRING: "STRING",
  },
...
class TestAutomationExecutionLogModel {
  id?: number; automation_id!: number; trigger_id!: number;
  action_id!: number; status!: string; error_message?: string;
  input_data?: object; output_data?: object;
  executed_at?: Date; created_at?: Date;
...
describe("AutomationExecutionLogModel", () => {
  it("should instantiate with required fields", () => {
    const log = new TestAutomationExecutionLogModel({
      id: 1, automation_id: 10, trigger_id: 20, action_id: 30, status: "success",
    });
  ...
  it("should store error information", () => {
    const log = new TestAutomationExecutionLogModel({
      automation_id: 1, trigger_id: 1, action_id: 1,
  ...
  it("should store input/output data", () => {
    const log = new TestAutomationExecutionLogModel({
      automation_id: 1, trigger_id: 1, action_id: 1, status: "success",
