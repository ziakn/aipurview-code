...
jest.mock("sequelize-typescript", () => ({
  Column: jest.fn(), DataType: {
    INTEGER: "INTEGER", TEXT: "TEXT", DATE: "DATE", JSONB: "JSONB", NOW: "NOW",
  },
...
class TestAutomationActionModel {
  ...
  default_params?: Record<string, any>; created_at?: Date; updated_at?: Date;

  constructor(data?: any) { if (data) Object.assign(this, data); }

  static async createNewAutomationAction(
    key: string, label: string, description?: string, default_params?: Record<string, any>,
    ...
    a.key = key.trim(); a.label = label.trim();
    a.description = description?.trim(); a.default_params = default_params || {};
    return a;
  }

  async updateAction(updateData: { label?: string; description?: string; default_params?: Record<string, any> }) {
    ...
    if (updateData.default_params !== undefined) { this.default_params = updateData.default_params; }
  }

  getSummary() { return { key: this.key, label: this.label, description: this.description }; }
  ...
  toJSON() {
    return { id: this.id, key: this.key, label: this.label, description: this.description, default_params: this.default_params };
  }
}
...
describe("AutomationActionModel", () => {
  describe("createNewAutomationAction", () => {
    it("should create with valid data", async () => {
      const a = await TestAutomationActionModel.createNewAutomationAction("send_email", "Send Email", "Sends an email");
    ...
    it("should throw for empty key", async () => {
      await expect(TestAutomationActionModel.createNewAutomationAction("", "Label")).rejects.toThrow(ValidationException);
    });

    it("should throw for empty label", async () => {
      await expect(TestAutomationActionModel.createNewAutomationAction("key", "")).rejects.toThrow(ValidationException);
    });
  });
