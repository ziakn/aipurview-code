...
class TestReportingModel {
  id?: number; organization_id!: number; report_type!: string;
  report_config?: object; generated_by!: number;
  created_at?: Date;
...
describe("ReportingModel", () => {
  it("should instantiate with required fields", () => {
    const r = new TestReportingModel({
      id: 1, organization_id: 1, report_type: "compliance_summary",
      generated_by: 42,
    });
  ...
  it("should store report configuration", () => {
    const r = new TestReportingModel({
      organization_id: 1, report_type: "risk_report", generated_by: 1,
