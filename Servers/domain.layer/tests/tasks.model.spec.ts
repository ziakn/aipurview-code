...
class TestTasksModel {
  ...
  project_id!: number; created_by!: number;
...
describe("TasksModel", () => {
  it("should instantiate with required fields", () => {
    const t = new TestTasksModel({
      id: 1, title: "Review AI model", status: TaskStatus.OPEN,
      priority: TaskPriority.HIGH, project_id: 10, created_by: 42,
    });
  ...
  it("should support all statuses", () => {
    Object.values(TaskStatus).forEach((status) => {
      const t = new TestTasksModel({
        title: "T", status, priority: "Low", project_id: 1, created_by: 1,
      });
  ...
  it("should support all priorities", () => {
    Object.values(TaskPriority).forEach((priority) => {
      const t = new TestTasksModel({
        title: "T", status: "Open", priority, project_id: 1, created_by: 1,
      });
  ...
  it("should handle due date", () => {
    ...
    const t = new TestTasksModel({
      ...
      project_id: 1, created_by: 1, due_date: dueDate,
    });
