import { AIIncidentManagementStatus, Severity, IncidentType } from "../enums/ai-incident-management.enum";

jest.mock("sequelize-typescript", () => ({
  Column: jest.fn(), DataType: {
    INTEGER: "INTEGER", STRING: jest.fn(() => "STRING"), TEXT: "TEXT",
    DATE: "DATE", BOOLEAN: "BOOLEAN", ENUM: jest.fn(), NOW: "NOW",
  },
...
describe("IncidentManagementModel", () => {
  it("should instantiate with required fields", () => {
    const incident = new TestIncidentManagementModel({
      id: 1, title: "Model drift detected",
  ...
  it("should support all statuses", () => {
    Object.values(AIIncidentManagementStatus).forEach((status) => {
      const incident = new TestIncidentManagementModel({
        title: "Test", status, severity: Severity.MINOR,
        incident_type: IncidentType.MALFUNCTION, reported_by: 1,
      });
  ...
  it("should support all severity levels", () => {
    Object.values(Severity).forEach((sev) => {
      const incident = new TestIncidentManagementModel({
        title: "Test", status: AIIncidentManagementStatus.OPEN,
        severity: sev, incident_type: IncidentType.MALFUNCTION, reported_by: 1,
      });
  ...
  it("should support all incident types", () => {
    Object.values(IncidentType).forEach((type) => {
      const incident = new TestIncidentManagementModel({
        title: "Test", status: AIIncidentManagementStatus.OPEN,
        severity: Severity.MINOR, incident_type: type, reported_by: 1,
      });
  ...
  it("should handle optional fields", () => {
    const incident = new TestIncidentManagementModel({
      title: "Test", status: AIIncidentManagementStatus.OPEN,
      severity: Severity.MINOR, incident_type: IncidentType.MALFUNCTION, reported_by: 1,
    });
