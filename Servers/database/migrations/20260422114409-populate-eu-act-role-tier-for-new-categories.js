  ...
  async up(queryInterface) {
    ...
    try {
      ...
      for (const m of mappings) {
        ...
        if (!cat) {
          console.warn(`[populate-eu-act-role-tier] category order_no=${m.order_no} not found — skipping`);
          continue;
        }
        ...
        for (const roleName of m.roles) {
          ...
          if (!rid) {
            console.warn(`[populate-eu-act-role-tier] role '${roleName}' not in eu_act_roles — skipping`);
            continue;
          }
          await queryInterface.sequelize.query(
        ...
        if (!tid) {
          console.warn(`[populate-eu-act-role-tier] tier '${m.tier}' not in eu_act_risk_tiers — skipping`);
          continue;
        }
        await queryInterface.sequelize.query(
      ...
      await t.commit();
      console.log('[populate-eu-act-role-tier] populated 8 categories; articles corrected for order_no 13 and 16');
    } catch (err) {
      await t.rollback();
      throw err;
    }
