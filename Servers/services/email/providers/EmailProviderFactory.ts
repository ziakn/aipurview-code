...
export class EmailProviderFactory {
  ...
  private static createExchangeOnlineProvider(): EmailProvider {
    ...
    if (!user || !pass) {
      throw new Error(
        "Exchange Online configuration incomplete. Required: EXCHANGE_ONLINE_USER, EXCHANGE_ONLINE_PASS",
      );
    }
  ...
  private static createOnPremisesExchangeProvider(): EmailProvider {
    ...
    if (!host || !port || !user || !pass) {
      throw new Error(
        "On-Premises Exchange configuration incomplete. Required: EXCHANGE_ONPREM_HOST, EXCHANGE_ONPREM_PORT, EXCHANGE_ONPREM_USER, EXCHANGE_ONPREM_PASS",
      );
    }
  ...
  private static createAmazonSESProvider(): EmailProvider {
    ...
    if (!region || !accessKeyId || !secretAccessKey) {
      throw new Error(
        "Amazon SES configuration incomplete. Required: AWS_SES_REGION, AWS_SES_ACCESS_KEY_ID, AWS_SES_SECRET_ACCESS_KEY",
      );
    }
  ...
  private static createAzureCommunicationServicesProvider(): EmailProvider {
    ...
    if (!connectionString) {
      throw new Error(
        "Azure Communication Services configuration incomplete. Required: AZURE_COMMUNICATION_CONNECTION_STRING",
      );
    }
